...
function TwoNetAPI(env) {
	var that = this;
	var configuration = config[env];

    this.api_rev = 'revV';
    this.xmlns = 'urn:com.twonet.sp.cuc.revisionv';

    this.customer_id = config.customer_id;

    // defaults to dev environment
    this.auth_key = configuration.auth_key;
	...
	this.call = function(command, method, write_object, callback) {
	    ...
	    // console.log(write_object);

	    var qreq = https.request(request_object, function(qres) {
	        var body = '';
	        ...
	        qres.on('end',function() {
	            callback(qres.statusCode,body);
	        });
	    });
	    // initiate details query
	    qreq.write(write_object);
	    qreq.end();
	};

	this.parseQLCResults = function(status, result, callback) {
		if( result.indexOf(this.xmlns) < 0 ) {
			if( status !== 200 ) {
				logme.error('QLC call failed with an empty return object : ' + status);
		...
		parser.parseString(result, function(err,jsonResult) {
			...
			if( status !== 200 ) {
				if( err ) {
					logme.error('Unable to parse the QLC error data. Impossible!');
				} else {
					logme.error('QLC call failed : ' + status);
					eyes.inspect(jsonResult);
				}
				callback(-1, {});
			} else if( err ) {  // API success but parsing failed
				logme.error('Unable to parse QLC results : ' + err);
				eyes.inspect(jsonResult);
				callback(-1, {});
			} else {
	...
	this.deviceIDRequest = function(model_name, mac, sensor_type) {
	    return('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
	...
	this.deviceCommandTrackingRequest = function(command_id) {
	    return('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
		    ...
		    + '<trackingNumber>' + command_id + '</trackingNumber></DeviceCommandTrackingNumber>'
		);
	};
...
TwoNetAPI.prototype.getHubs = function(callback) {
	...
	var record_range = function(start, count) {
        return('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
	    + '<RecordRange xmlns="urn:com.twonet.sp.cuc.revisionv">'
    ...
    var do_work = function(status,result) {
        parser.parseString(result,function(err,jsonResult) {
            ...
            logme.debug('fetching from hub list...');
	...
	async.parallel({
		...
		hub_details : function(done) {
		    that.call('/hub/retrieve', 'POST', that.hubRequest(hub_id), function(status,details) {
				that.parseQLCResults(status,details,done);
    		});
		},
		...
		device_list : function(done) {
			that.call('/device/registeredDevices/retrieve', 'POST', that.hubDevicesRequest(hub_id,0,100),function(status,devices) {
				that.parseQLCResults(status,devices,done);
            });
		}
...
TwoNetAPI.prototype.activateHub = function(hub_id, callback) {
	...
	this.call('/hub/activation', 'PUT', this.hubHardwareRequest(true, hub_id), function(status, result) {
		that.parseQLCResults(status, result, callback);
	});
};
...
TwoNetAPI.prototype.activateHubStatus = function(hub_id, callback) {
	var that = this;
    that.call('/hub/retrieve', 'POST', that.hubRequest(hub_id), function(status,details) {
		that.parseQLCResults(status,details,callback);
	});
};

TwoNetAPI.prototype.activateDevice = function(model_name, mac, sensor_type, callback) {
	var that = this;
	this.call('/device/activation', 'PUT', this.deviceIDRequest(model_name, mac, sensor_type), function(status, result) {
		that.parseQLCResults(status, result, callback);
	});
};

TwoNetAPI.prototype.deactivateDevice = function(model_name, mac, sensor_type, callback) {
	var that = this;
	this.call('/device/deactivation', 'PUT', this.deviceIDRequest(model_name, mac, sensor_type), function(status, result) {
		that.parseQLCResults(status, result, callback);
	});
};

TwoNetAPI.prototype.getDevice = function(model_name, mac, sensor_type, callback) {
	var that = this;
	this.call('/device/retrieve', 'POST', this.deviceIDRequest(model_name, mac, sensor_type), function(status, result) {
		that.parseQLCResults(status, result, callback);
	});
};
